...
// BUGFIX: java.net.HttpCookie hasExpires implementation will return always FALSE
// @link https://code.google.com/p/android/issues/detail?id=191981
 ...
 * @since 1.6
 */
public final class HttpCookie extends ParcelableObject
        implements Cloneable, Serializable
{
    private static final long serialVersionUID = -1725243885034790181L;
    ...
    static class CookieParser {
        ...
        private void setAttribute(HttpCookie cookie, String name, String value) {
            ...
            } else if (name.equals("max-age") && cookie.maxAge == MAX_AGE_UNSPECIFIED) {
    ...
    /**
     * Since the positive and zero max-age have their meanings, this value
     * serves as a hint as 'not specify max-age'.
     */
    private final static long MAX_AGE_UNSPECIFIED = -1L;
    ...
    public boolean hasExpired(final Date date) {
        long currentTime = (date != null) ? date.getTime() : System.currentTimeMillis();
        return (cookieExpiryDate != null) && (cookieExpiryDate.getTime() <= currentTime);
    }
    ...
    /**
     * Sets the expiration {@link Date} after which this cookie is no longer valid.
     */
    public void setExpiryDate(final Date expiryDate)
    {
        if (expiryDate != null) {
            long deltaSeconds = (expiryDate.getTime() - System.currentTimeMillis()) / 1000;

            maxAge = (deltaSeconds > 0) ? deltaSeconds : 0;
    ...
    {
        if (deltaSeconds >= 0) {
            Date expiryDate = new Date(System.currentTimeMillis() + deltaSeconds * 1000);
    ...
    @Override public boolean equals(Object object) {
        ...
        if (object instanceof HttpCookie) {
            ...
            return name.equalsIgnoreCase(that.getName())
                    ...
                    && ObjectUtils.nullSafeEquals(path, that.path);
        }
        return false;
    }
...
// MARK: - Serialization: Parcelable

    public static final Parcelable.Creator<HttpCookie> CREATOR = new Parcelable.Creator<HttpCookie>()
    {
        @Override
        public HttpCookie createFromParcel(Parcel in) {
    ...
    private HttpCookie(Parcel src) {
        // Deserialize object
        setName(src.readString());
        setValue(src.readString());
        setComment(src.readString());
        setCommentURL(src.readString());
        setDomain(src.readString());
        long expiryTime = src.readLong();
        setExpiryDate(expiryTime >= 0 ? new Date(expiryTime) : null);
        setPath(src.readString());
        setPortlist(src.readString());
        setVersion(src.readInt());
        setSecure(readBooleanFromParcel(src));
        setDiscard(readBooleanFromParcel(src));
        setHttpOnly(readBooleanFromParcel(src));
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(getCommentURL());
        dest.writeString(getDomain());
        final Date expiryDate = getExpiryDate();
        dest.writeLong(expiryDate != null ? expiryDate.getTime() : -1L);
        dest.writeString(getPath());
        dest.writeString(getPortlist());
        dest.writeInt(getVersion());
        writeBooleanToParcel(dest, getSecure());
        writeBooleanToParcel(dest, getDiscard());
        writeBooleanToParcel(dest, isHttpOnly());
    }

// MARK: - Serialization: Serializable
            ...
            throws IOException, ClassNotFoundException
    {
        ...
        setCommentURL((String) in.readObject());
        setDomain((String) in.readObject());
        long expiryTime = in.readLong();
        setExpiryDate(expiryTime >= 0 ? new Date(expiryTime) : null);
        setPath((String) in.readObject());
        setPortlist((String) in.readObject());
        setVersion(in.readInt());
        setSecure(in.readBoolean());
        setDiscard(in.readBoolean());
            ...
            throws IOException
    {
        ...
        out.writeObject(getCommentURL());
        out.writeObject(getDomain());
        final Date expiryDate = getExpiryDate();
        out.writeLong(expiryDate != null ? expiryDate.getTime() : -1L);
        out.writeObject(getPath());
        out.writeObject(getPortlist());
        out.writeInt(getVersion());
        out.writeBoolean(getSecure());
        out.writeBoolean(getDiscard());
        out.writeBoolean(isHttpOnly());
    }
